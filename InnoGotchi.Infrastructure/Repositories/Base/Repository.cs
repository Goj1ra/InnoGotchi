...
namespace InnoGotchi.Infrastructure.Repositories.Base
{
    /// <summary>
    ...
    public sealed class Repository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        /// <param name="disableTracking"><c>true</c> to disable changing tracking; otherwise, <c>false</c>. Default to <c>true</c>.</param>
        /// <returns>The <see cref="IQueryable{TEntity}"/>.</returns>
        public async Task<List<TResult>> GetAllAsync<TResult>(Expression<Func<TEntity, TResult>> selector, bool disableTracking = true) =>
            disableTracking
